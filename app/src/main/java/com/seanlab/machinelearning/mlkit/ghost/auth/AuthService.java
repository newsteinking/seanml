...
import com.seanlab.machinelearning.mlkit.ghost.error.TokenRevocationFailedException;
import com.seanlab.machinelearning.mlkit.ghost.event.CredentialsExpiredEvent;
...
import com.seanlab.machinelearning.mlkit.ghost.network.entity.RefreshReqBody;
import com.seanlab.machinelearning.mlkit.ghost.network.entity.RevokeReqBody;
...
public class AuthService implements Listenable<AuthService.Listener> {
    ...
    private void handleLoginError(Throwable e) {
        ...
        if (NetworkUtils.isUnprocessableEntity(e) || NetworkUtils.isUnauthorized(e)) {
            ...
            BusProvider.getBus().post(new CredentialsExpiredEvent());
        } else {
            // deliberately missing mListener != null check, to avoid suppressing errors
            mListener.onUnrecoverableFailure();
        }
